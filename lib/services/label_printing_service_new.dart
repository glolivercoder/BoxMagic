...
class LabelPrintingService {
  ...
  double _calculateQrCodeSize(double labelHeight, double labelWidth) {
    ...
    // O QR code deve ocupar no máximo 70% da menor dimensão
  ...
  pw.Widget _buildLabelWidget(
    ...
    return pw.Container(
      ...
      child: _buildLabelContent(box, items, format, qrCodes, labelWidth, labelHeight),
    );
  }
    ...
    return pw.Column(
      ...
      children: [
        ...
        pw.Expanded(
          child: pw.Center(
            child: qrCode != null
                ? pw.Image(pw.MemoryImage(qrCode), width: qrCodeSize, height: qrCodeSize)
                : pw.Container(),
          ),
        ),
      ],
  ...
  pw.Widget _buildIdWithBarcodeAndItems(Box box, List<Item> items, Map<int, Uint8List> qrCodes, double labelWidth, double labelHeight) {
    final qrCodeSize = _calculateQrCodeSize(labelHeight * 0.6, labelWidth * 0.4); // QR code menor neste formato
    final qrCode = box.id != null ? qrCodes[box.id] : null;
      ...
      children: [
        pw.Row(
          ...
          children: [
            ...
            qrCode != null
                ? pw.Image(pw.MemoryImage(qrCode), width: qrCodeSize, height: qrCodeSize)
                : pw.Container(width: qrCodeSize, height: qrCodeSize),
          ],
        ),
        pw.SizedBox(height: 5),
        pw.Text(
          'Itens:',
          style: pw.TextStyle(
            fontSize: _calculateFontSize(labelHeight, 10),
            fontWeight: pw.FontWeight.bold,
          ),
        ),
        pw.SizedBox(height: 2),
        pw.Container(
          height: labelHeight * 0.4, // Limitar altura da lista de itens
          child: pw.ListView(
            children: [
